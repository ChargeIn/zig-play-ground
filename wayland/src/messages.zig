...
/// C ABI compatible header for wayland packets
pub const Header = extern struct {
    ...
    pub fn read(socket: std.net.Stream) !Header {
        ...
        const header_bytes = try socket.readAll(std.mem.asBytes(&header));
...
pub const Event = struct {
    ...
    pub fn read(socket: std.net.Stream, body_buffer: *std.ArrayList(u8)) !Event {
        ...
        // read bytes until we match the size in the header, not including the bytes the header.
        try body_buffer.resize(header.size - @sizeOf(Header));

        const message_bytes = try socket.readAll(body_buffer.items);

        if (message_bytes < body_buffer.items.len) {
            return error.UnexpectedEOF;
        }
        return Event{ .header = header, .body = body_buffer.items };
    }

    /// Handles creating a header and writing the request to the socket.
    pub fn write(socket: std.net.Stream, object_id: u32, opcode: u16, message: []const u32) !void {
...
/// https://wayland.app/protocols/wayland#wl_shm
